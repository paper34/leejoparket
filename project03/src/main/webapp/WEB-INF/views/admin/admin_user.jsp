	...
	<script type="text/javascript">
		...
		$.ajax({
		   ...
		   $('.delete').click(function() {
			 ...
			 if(result == true){
			  	location.href = 'admin_userdelete.do?user_id='+user_id
			  }
			  else{
				  return false;
			  } 
 ...
 -->
				...
				<div class="container">
					...
					<tr>
						...
						<th style="background-color: #eeeeee; text-align: center;">블랙여부</th>
				...
				<c:forEach var="vo" items="${list}">
				   	<c:choose>
					<c:when test="${vo.user_black == '1'}">
						<c:set var="service" value="블랙회원"></c:set>
					</c:when> 
					<c:otherwise>
						<c:set var="service" value="정상회원"></c:set>
					</c:otherwise> 
					</c:choose>
					<tr>
					  <td>${vo.admin_userno}</td>
					  <td>${vo.user_id}</td>
					  <td>${vo.user_name}</td>
					  <td>${vo.user_date}</td>
					  <td>${service}</td>
					  ...
					  <button class="btn btn-danger delete"  id="${vo.user_id}">탈퇴</button>
					  </td>
                    </tr>
                 </c:forEach>   
