...
<link href="../resources/bootstrap/css/sb-admin-2.min.css" rel="stylesheet">
	...
	<script type="text/javascript">
		...
		$('.detail').click(function() {
			var idValue = $(this).attr('id') //id값 얻어오기

		$.ajax({
		    url:"admin_bedalajax.do",
		    data: {
		        bedal_list_id : idValue
 ...
 -->
				...
				<div class="container">
					...
					<div
						...
						<h1 class="h3 mb-0 text-gray-800">배달내역관리</h1>
					</div>
					...
					<tr>
						...
						<th style="background-color: #eeeeee; text-align: center;">배달유저</th>
						<th style="background-color: #eeeeee; text-align: center;">유저</th>
						<th style="background-color: #eeeeee; text-align: center;">배달지역</th>
						<th style="background-color: #eeeeee; text-align: center;">상품제목</th>
						<th style="background-color: #eeeeee; text-align: center;">상세보기</th>
				...
				<c:forEach var="vo" items="${list}">
					...
					<tr>
					  <td>${vo.bedal_list_id}</td>
					  <td>${vo.bedal_id}</td>
					  <td>${vo.user_id}</td>
					  <td>${vo.user_address}</td>
					  <td>${vo.product_title}</td>
					  <td>
					  <button class="btn btn-success detail" data-toggle="modal"  href="#listModal" id="${vo.bedal_list_id}">상세</button>
					  </td>
					  <td>
					  <c:choose>
					<c:when test="${vo.bedal_list_status == '1'}">
						<c:set var="service" value="배송완료"></c:set>
					</c:when> 
					<c:when test="${vo.bedal_list_status == '-1'}">
						<c:set var="service" value="배송전"></c:set>
					</c:when>
					<c:when test="${vo.bedal_list_status == '-2'}">
						<c:set var="service" value="배송거부"></c:set>
					</c:when>  
					<c:otherwise>
				 	...
				 	<div class="modal-dialog">
				 		<div class="modal-content" id="modalcontent">
				 			<div class="modal-header">
				 					<h5 class="modal-title" id="modal">배달내역상세</h5>
