		...
		$.ajax({
		  ...
		  $('.delete').click(function() {
			 result = confirm('삭제하시겠습니까?');
			  qa_no = $(this).attr('id')
			 if(result == true){
			  	location.href = 'admin_questiondelete.do?qa_no='+qa_no
			  }
			  else{
				  return false;
			  } 
 ...
 -->
				...
				<div class="container">
					...
					<div
						...
						<h1 class="h3 mb-0 text-gray-800">Q/A게시판관리</h1>
					</div>
					...
					<tr>
						...
						<th style="background-color: #eeeeee; text-align: center;">기록시간</th>
                  ...
                  <tbody>
				<c:forEach var="vo" items="${list}"> <!-- 3개 테이블 join  -->
				    <c:choose>
					...
					<c:when test="${vo.qa_status == '1'}">
						<c:set var="status" value="답변완료"></c:set>
					</c:when> 
					<c:otherwise>
						<c:set var="status" value="미답변"></c:set>
					</c:otherwise> 
					
					</c:choose>
					<tr>
					  <td>${vo.admin_question_no}</td>
					  <td>${service}</td>
					  <td>${vo.user_id}</td>
					  <td>${vo.user_name}</td>
					  <td>${vo.qa_date}</td>
					  ...
					  <button class="btn btn-danger delete" id="${vo.qa_no}">삭제</button>
					  </td>
                    </tr>
                 </c:forEach>   
