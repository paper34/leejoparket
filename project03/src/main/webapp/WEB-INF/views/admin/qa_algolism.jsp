...
<link href="../resources/bootstrap/css/sb-admin-2.min.css" rel="stylesheet">
	...
	<script type="text/javascript">
		...
		$('.detail').click(function() {
			qaValue = $(this).attr('id') //id값 얻어오기
		$.ajax({
		    url:"qa_algolismajax.do",
		    data: {
		       qa_no: qaValue
		    },
...
<body id="page-top">
	...
	<div id="wrapper">
		...
		<ul class="navbar-nav bg-gradient-primary sidebar sidebar-dark accordion" id="accordionSidebar">
           ...
           <!-- Divider -->
			...
			<div class="sidebar-heading"><h4 style="font-weight:bold">추천Q/A게시판</h4></div>
 ...
 -->
				...
				<!-- Begin Page Content -->
				<div class="container">
				<fmt:parseNumber var= "ages" integerOnly= "true" value= "${list[0].qa_age / 10}"/>
			  <c:choose>
					<c:when test="${ages == '1'}">
						<c:set var="service" value="10대"></c:set>
					</c:when> 
					<c:when test="${ages == '2'}">
						<c:set var="service" value="20대"></c:set>
					</c:when>
					<c:when test="${ages == '3'}">
						<c:set var="service" value="30대"></c:set>
					</c:when>  
					<c:when test="${ages == '4'}">
						<c:set var="service" value="40대"></c:set>
					</c:when>  
					<c:when test="${ages == '5'}">
						<c:set var="service" value="50대"></c:set>
					</c:when>  
					...
					<div
						...
						<h1 class="h3 mb-0 text-gray-800">${service} Q/A추천게시물</h1>
					</div>
					...
					<tr>
						...
						<th style="background-color: #eeeeee; text-align: center;">상세</th>
					</tr>
                  </thead>
                  <tbody>
				<c:forEach var="vo" items="${list}"> 
					<tr>
					  ...
					  <td>${vo.qa_title}</td>
					  <td>
					  <button class="btn btn-success detail" data-toggle="modal"  href="#listModal" id="${vo.qa_no}">상세</button>
					  </td>
                    </tr>
                 </c:forEach>   
